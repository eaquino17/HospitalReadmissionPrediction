...
// Removed non-existent sklearn-js import
// Removed non-existent smote-js import
...
async function loadAndExploreData() {
  ...
  console.log(
    `- Gender distribution: ${((data.filter((p) => p.gender === "Male").length / data.length) * 100).toFixed(1)}% Male, ${((data.filter((p) => p.gender === "Female").length / data.length) * 100).toFixed(1)}% Female`,
  )
...
function prepareModelData(data) {
  ...
  // Implementing our own train_test_split since sklearn-js is not available
  ...
  if (imbalanceRatio > 1.5) {
    console.log(
      "Class imbalance detected. In a real implementation, we would apply techniques like SMOTE for oversampling the minority class.",
    )
  }
